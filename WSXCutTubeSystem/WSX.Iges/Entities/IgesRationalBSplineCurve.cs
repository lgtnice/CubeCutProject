...
namespace WSX.Iges.Entities
{
    ...
    public class IgesRationalBSplineCurve : IgesEntity
    {
        ...
        {
            float[] floats = new float[ControlPoints.Count * 3];
            int index = 0;
            foreach (IgesPoint pt in ControlPoints)
            {
                floats[index++] = (float)pt.X;
                floats[index++] = (float)pt.Y;
                floats[index++] = (float)pt.Z;
            }
